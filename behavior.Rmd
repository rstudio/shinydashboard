...
server <- function(input, output, session) {
  output$res <- renderText({
    paste("Sidebar is collapsed:", input$sidebarCollapsed)
  })
}
...
While it is possible to render dynamic dropdown menus (placed in the app's header), it's not possible to treat them as inputs. In other words, you can re-render a dynamic dropdown menu if the data changes. For example, in this app, we can dynamically add items to a `dropdownMenu` via a modal window:
